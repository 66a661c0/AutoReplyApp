...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadContacts()
        adapter = ContactAdapter(targetContacts) { position ->
    ...
    private fun removeContact(position: Int) {
        AlertDialog.Builder(this)
            .setTitle("삭제 확인")
            .setMessage("${targetContacts[position].name}을(를) 목록에서 제거할까요?")
            .setPositiveButton("삭제") { _, _ ->
                targetContacts.removeAt(position)
    ...
    private fun updateCount() {
        tvCount.text = "대상 번호 (${targetContacts.size}명)"
    }
}
